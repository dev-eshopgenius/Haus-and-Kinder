...
class SliderComponent extends HTMLElement {
  constructor() {
    ...
    this.dots = this.querySelectorAll(".slide_dot");
  ...
  onDotClick(event) {
    event.preventDefault();
    const step = event.currentTarget.dataset.step || 1;
    const dots = event.currentTarget.closest(".slide_dots");
    const slideId = event.currentTarget.getAttribute('aria-controls');
    dots
      .querySelectorAll(".slide_dot")
      .forEach((dot) => dot.classList.remove("active"));
    event.currentTarget.classList.add('active');
    this.slideScrollPosition = document.querySelector(slideId)?.scrollLeft || 0;
    console.log(step, "<<<step",this.slideScrollPosition);
...
class VariantSelects extends HTMLElement {
  ...
  onVariantChange() {
    ...
    try {
      document.querySelector(`.sticky--atc-wrapper [name="id"]`).value =
...
class VariantRadios extends VariantSelects {
  ...
  updateOptions() {
    ...
    document
      .querySelector(
        `.sticky--atc-button [type="radio"][value="${this.options[0]}"]`
      )
      .click();
  }
}
